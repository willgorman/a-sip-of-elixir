...
"""
  ...
  @dirs [-@row-1, -@row+1, -2, 2, @row-1, @row+1]
  ...
  def solve(board) do
    receive do
      from ->
        ...
        case result do
          {:so_much_win, boards} ->
            from <- {:so_much_win, Enum.concat(boards, [board])}
          _ ->
  ...
  def wait(from, board, spawned) do
    receive do
      {:so_much_win, boards} ->
        all_boards = Enum.concat(boards, [board])
        IO.inspect(Enum.count(all_boards))
        if (Enum.count(all_boards) == 14) do
          IO.puts(all_boards)
          IO.puts("DONE!")
          :init.stop()
        else
